...
<form action="{{ route('admin-user-update', $user->id) }}" method="post" enctype="multipart/form-data"
    ...
    <div class="row mt-2">
        ...
        <div class="form-group col-md-4">
            ...
            <input id="middle_name" name="middle_name" type="text" class="form-control" placeholder="Nickname"
                value="@if (@$user->info->mid_name) {{ @$user->info->mid_name }}@else{{ old('mid_name') }} @endif">
        ...
        <div class="form-group col-md-4">
            <label for="password" class="control-label mb-1">Password <small class="text-info">Password length
                    minimum 6</small></label>
        ...
        <div class="form-group col-md-4">
            ...
            <select name="role" class="form-control">
                ...
                @foreach ($roles as $role)
                    <option value="{{ $role->id }}" @if ($role->id == @$user->role_id || $role->id == old('role')) selected @endif>
                        {{ $role->name }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group col-md-4">
            ...
            <select name="company" class="form-control">
                ...
                @foreach ($companies as $company)
                    <option value="{{ $company->id }}" @if ($company->id == @$user->company_id || $company->id == old('company')) selected @endif>
    ...
    <div class="row mt-2">
        ...
        <div class="form-group col-md-4">
            ...
            <select name="country" id="country" class="form-control">
                ...
                @foreach ($countries as $country)
                    <option value="{{ $country->id }}" @if ($country->id == @$user->info->country_id) selected @endif>
                        {{ @$country->name }}</option>
                @endforeach
            </select>
        ...
        <div class="form-group col-md-6">
            <label for="calling_code" class="control-label mb-1">Phone Number <small
            ...
            <div class="row">
                <div class="col-md-4">
                    <select name="calling_code" id="calling_code" class="form-control" required="">
                        <option value="">Select Country</option>
                        @foreach ($countries as $country)
                            <option value="{{ $country->id }}" @if (@$country->id == @$user->info->calling_code) selected @endif>
