                ...
                <form action="{{ url('admin/work/store') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="form-group col-md-12">
                            <strong for="question" class="control-label mb-1">Title<small
                                    class="text-danger">*</small></strong>
                            <input id="title" name="title" type="text" class="form-control" aria-required="true"
                                aria-invalid="false" placeholder="Title" required="" value="{{ old('title') }}">
                            <span class="text-danger"> {{ $errors->first('title') }}</span>
                        </div>
                        <div class="form-group col-md-12">
                            <strong for="question" class="control-label mb-1">Desc</strong>
                            <input id="desc" name="desc" type="text" class="form-control" placeholder="Desc"
                                value="{{ old('desc') }}">
                            <span class="text-danger"> {{ $errors->first('desc') }}</span>
                        </div>

                        <div class="form-group col-md-12">
                            <strong for="question" class="control-label mb-1">Job image</strong>
                            <input id="thumbnail" name="thumbnail" type="file" class="form-control"
                                placeholder="thumbnail" value="{{ old('thumbnail') }}">
                        </div>

                        <div class="form-group col-md-6 mt-3">
                            <strong for="category" class="control-label mb-1">Select Category<small
                                    class="text-danger">*</small></strong>
